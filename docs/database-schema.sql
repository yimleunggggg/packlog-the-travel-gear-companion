...
-- RLS: cloud snapshots are scoped to the signed-in user's workspace (`u:<auth.uid()>`).
-- Guests use browser localStorage and must not read/write a shared Supabase workspace.
