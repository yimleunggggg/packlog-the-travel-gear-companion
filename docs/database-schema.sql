...
-- RLS: snapshots are only available to the signed-in user's own workspace (`u:<auth.uid()>`).
-- Anonymous sessions stay in browser storage and cannot read/write shared cloud snapshots.
