...
-- RLS: authenticated users can only access their own `u:<auth.uid()>` snapshot.
-- Anonymous sessions persist to browser localStorage and must not share a Supabase workspace.
