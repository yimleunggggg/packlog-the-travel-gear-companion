...
**请在 Supabase SQL Editor 中执行一次该文件全文**（或 `supabase db push` 若你使用 Supabase CLI 关联项目）。**CI 自动迁移：** 在 GitHub 配置 `SUPABASE_*` 三个 Secret 后，每次 `main` 部署前会执行 `supabase db push`，见 **[docs/CI_DEPLOY.md](CI_DEPLOY.md)**。
