...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const coordinator = createAuthBootCoordinator({
      applySession,
      markReady: () => setReady(true),
    });

    /** 弱网/墙内 Supabase 慢或挂起时，避免 AuthGate 永久 disabled。 */
    const bootTimer = window.setTimeout(() => coordinator.timeout(), 8000);

    client.auth
      .getSession()
      .then(({ data }) => {
        window.clearTimeout(bootTimer);
        coordinator.getSessionResolved(data.session);
      })
      .catch(() => {
        window.clearTimeout(bootTimer);
        coordinator.getSessionRejected();
      });

    const { data: sub } = client.auth.onAuthStateChange((_event, nextSession) => {
      coordinator.authStateChanged(nextSession);
    });
