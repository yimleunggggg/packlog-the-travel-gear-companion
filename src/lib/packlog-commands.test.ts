...
describe("moveTripItem", () => {
  it("keeps the source item when the target container is missing", () => {
    const trip = diveTripWithItem();

    const next = moveTripItem(trip, "c1", "i-cert", "missing");

    expect(next).toBe(trip);
    expect(next.containers[0]!.items.map((i) => i.id)).toEqual(["i-cert"]);
  });

  it("creates the unassigned bucket before moving into it", () => {
    const trip = diveTripWithItem();

    const next = moveTripItem(trip, "c1", "i-cert", unassignedContainerId(trip.id));

    expect(next.containers.find((c) => c.id === "c1")!.items).toHaveLength(0);
    expect(next.containers.find((c) => c.id === unassignedContainerId(trip.id))!.items[0]!.id).toBe(
      "i-cert",
    );
  });
});
