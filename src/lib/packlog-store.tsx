...
export function PacklogProvider({ children }: { children: ReactNode }) {
  ...
  const repositoryTarget = useMemo(
    () => (authReady ? resolvePacklogRepositoryTarget({ userId: user?.id ?? null }) : null),
    [authReady, user?.id],
  );
  const repositoryKey = repositoryTarget ? packlogRepositoryTargetKey(repositoryTarget) : null;
  const repository = useMemo(
    () =>
      repositoryTarget ? createPacklogRepository(seedForRepo, { userId: user?.id ?? null }) : null,
    [repositoryTarget, seedForRepo, user?.id],
  );
  ...
  useEffect(() => {
    if (!repository || !repositoryKey) return;
    let alive = true;
  ...
  useEffect(() => {
    if (!repository || !repositoryKey || hydratedRepositoryKey !== repositoryKey) return;
