      ...
      {phase !== "REVIEW" ? (
        ...
        >
          ...
          {variant === "page" && isDesktop ? (
            <Collapsible
              open={categoryChecklistOpen}
              onOpenChange={setCategoryChecklistOpen}
              className="group md:col-span-2 mt-2 border-t border-border pt-4"
            >
              <CollapsibleTrigger
                type="button"
                className="flex w-full min-h-11 items-center justify-between gap-3 rounded-md border border-border-strong bg-surface/70 px-3 py-2 text-left font-mono text-[10px] tracking-[0.18em] text-muted-foreground transition hover:border-foreground/25 hover:text-foreground md:min-h-0"
              >
                <span>{t("pack.page.categoryChecklistFold")}</span>
                <ChevronDown
