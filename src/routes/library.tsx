...
import { packlogHint, packlogKicker, packlogPageTitle } from "@/lib/packlog-button-classes";
...
function LibraryPage() {
  ...
  return (
    <div className="min-h-dvh pb-[max(1.25rem,env(safe-area-inset-bottom))]">
      ...
      <main className="mx-auto max-w-[1480px] space-y-6 px-[var(--page-padding)] py-6 md:py-8">
        <header className="flex items-end justify-between gap-4">
          <div>
            <div className={cn(packlogKicker, "text-signal")}>{t("library.head")}</div>
            <h1 className={cn("mt-2", packlogPageTitle)}>{t("library.title")}</h1>
            <p className={cn("mt-2 max-w-prose text-pretty", packlogHint)}>
              {t("library.subtitle")}
            </p>
          </div>
        </header>
