-- Lock down full-state snapshots now that browser auth is enabled.
-- Anonymous users stay on localStorage; Supabase rows are only for authenticated
-- workspaces named `u:<auth.uid()>`.
